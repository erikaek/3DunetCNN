...
def definefolders():
    ...
    data_folders = [part1_part2_fold, part3_fold, part4_fold];

    return organs, part_folders, data_folders

organs, part_folders, data_folders = definefolders()

for df in data_folders[:2]:

    for pf in part_folders:
        readpath = '../../Data/'+df+'/'+pf+'/'
        savepath = 'data/original/'+pf+'/'

        if os.path.exists(readpath+'structures/'+organs[3]+'.nrrd'):
            label_data, info = readnrrd(readpath+'structures/'+organs[3]+'.nrrd')
            writenifti(label_data, savepath + 'img.nii', info)
            ct_data, info = readnrrd(datapath+'img.nrrd')
            writenifti(ct_data, savepath+'img.nii', info)
